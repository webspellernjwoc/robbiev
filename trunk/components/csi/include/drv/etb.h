...
typedef enum {
    ETB_ONE_TRIGGER_ONE	= 0,      	///< one device trig one deivce
    ETB_ONE_TRIGGER_MORE,          	///< one device trig two for more device
    ETB_MORE_TRIGGER_ONE           	///< two or more device trig one deivce
} csi_etb_ch_type_e;
...
typedef struct {
	...
	uint8_t               dst_ip1;
	uint8_t               dst_ip2;  
    csi_etb_trig_mode_e   trig_mode;  ///< the input source is hardware trigger or software trigger.
    csi_etb_ch_type_e     ch_type;    ///< channel type
} csi_etb_config_t;
